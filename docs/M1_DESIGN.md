...
```text
Snapshot  S<n>   不可变的整树快照(init / checkpoint 的产物),内部路径,per-file uchg 保护
...
| 原语 | 实现 | 成本(27.0 实测) |
|---|---|---|
| `init <dir>` | `clonefile(dir)` → S1;原目录不动、不接管 | 50k 文件 0.37s + 保护 ~0.4s |
...
├── snapshots/S<n>/root/         uchg 保护;目录本身 0500
...
| P3 | 快照被改动,污染后代 | 每个文件/目录 `chflags uchg`,目录 0500;fork 时在克隆上并行 `nouchg`。`world fs verify S<n>` 用清单(路径、大小、mtime、可选 hash)校验 |
| P4 | 误删 World | `discard` = rename 进 trash,`world fs restore W<n>` 可恢复;保留期后 `gc` 真删;`--now` 才立即删 |
| P5 | 对正在使用的 World 做 discard/checkpoint/fork | `world exec` 写 `.world/lock`(pid);discard 拒绝活锁;fork/checkpoint 提示写者存在(目录克隆原子,但有 10–15ms 尖峰),`--force` 继续 |
...
- T1.1 core 重构:schema v2、snapshot/world/pool/trash 生命周期、平台原语、P6/P8/P12/P13
- T1.2 CLI:init/fork/checkpoint/list/inspect/discard/restore/gc/status/verify/adopt,错误信息可读,P1/P2/P4/P7/P11
- T1.3 diff:FSEvents 层 + 全扫回退 + 比对,P10;输出格式同 arch.md §25
- T1.4 exec:cwd/env/lock/seatbelt profile,P5/P14
